        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
            <div className="p-3.5 rounded-2xl bg-surface-elevated border border-border space-y-1">
              <div className="text-[11px] font-medium text-muted">Reliability Score</div>
              <div className="text-xl font-bold text-success font-mono">{telemetry.reliabilityScore}%</div>
              <div className="text-[10px] text-success">System reliability score</div>
            </div>

            <div className="p-3.5 rounded-2xl bg-surface-elevated border border-border space-y-1">
              <div className="text-[11px] font-medium text-muted">Core API Availability</div>
              <div className="text-xl font-bold text-white font-mono">99.8%</div>
              <div className="text-[10px] text-success">Target &gt;= 99.5%</div>
            </div>

            <div className="p-3.5 rounded-2xl bg-surface-elevated border border-border space-y-1">
              <div className="text-[11px] font-medium text-muted">Database Queries</div>
              <div className="text-xl font-bold text-info font-mono">{telemetry.databaseHealth.totalQueries}</div>
              <div className="text-[10px] text-info">24h telemetry window</div>
            </div>

            <div className="p-3.5 rounded-2xl bg-surface-elevated border border-border space-y-1">
              <div className="text-[11px] font-medium text-muted">Webhook Success Rate</div>
              <div className="text-xl font-bold text-success font-mono">{telemetry.webhookHealth.successRatePercent}%</div>
              <div className="text-[10px] text-success">Razorpay payments ledger</div>
            </div>

            <div className="p-3.5 rounded-2xl bg-surface-elevated border border-border space-y-1">
              <div className="text-[11px] font-medium text-muted">Workflow Cron Health</div>
              <div className="text-xl font-bold text-success font-mono">{telemetry.cronHealth.workflowCronStatus}</div>
              <div className="text-[10px] text-success">5m schedule active</div>
            </div>

            <div className="p-3.5 rounded-2xl bg-surface-elevated border border-border space-y-1">
              <div className="text-[11px] font-medium text-muted">SLA Cron Health</div>
              <div className="text-xl font-bold text-success font-mono">{telemetry.cronHealth.slaCronStatus}</div>
              <div className="text-[10px] text-success">5m evaluation active</div>
            </div>

            <div className="p-3.5 rounded-2xl bg-surface-elevated border border-border space-y-1 col-span-1 sm:col-span-2 lg:col-span-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-success text-sm">shield_with_heart</span>
                  <span className="text-xs font-semibold text-white">Disaster Recovery Readiness Posture:</span>
                  <span className="text-xs font-mono font-bold text-success">READY (RTO &lt;= 60m, RPO &lt;= 15m, 100% Migration Score)</span>
                </div>
          ...
          <div className="p-6 rounded-2xl bg-surface-elevated border border-border space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-muted">{telemetry.sloResults.length} Production Targets</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {telemetry.sloResults.map((slo) => (
                <div key={slo.name} className="p-4 rounded-xl bg-black/40 border border-border space-y-2">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="text-xs font-bold text-white">{slo.name}</div>
                      <div className="text-[10px] text-muted font-mono">Target: &gt;= {slo.targetPercent}%</div>
                    </div>
                  ...
                  <div className="flex items-end justify-between font-mono">
                    ...
                    <span className="text-[11px] text-muted">Budget: {slo.errorBudgetRemainingPercent}%</span>
                  </div>
          ...
          <div className="p-6 rounded-2xl bg-surface-elevated border border-border space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-white flex items-center gap-2">
                <span className="material-symbols-outlined text-info">api</span>
                API Service Latency &amp; Error Rate Telemetry
              </h2>
              <span className="text-xs font-mono text-muted">24-Hour Bounded Window</span>
            </div>
                ...
                <tbody className="divide-y divide-border">
                  {telemetry.apiHealth.map((srv) => (
                    <tr key={srv.serviceName} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="py-3 px-3 text-center font-mono text-success">{srv.successRatePercent}%</td>
                      <td className="py-3 px-3 text-center font-mono text-muted">{srv.errorRatePercent}%</td>
                      <td className="py-3 px-3 text-center font-mono">{srv.avgDurationMs} ms</td>
                      <td className="py-3 px-3 text-center font-mono text-warning">{srv.p95DurationMs} ms</td>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="p-5 rounded-2xl bg-surface-elevated border border-border space-y-3">
              <h2 className="text-sm font-semibold text-white flex items-center gap-2">
                <span className="material-symbols-outlined text-warning text-base">database</span>
                Database Query Performance by Category
              </h2>
              <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                <div className="p-3 rounded-xl bg-black/40 border border-border space-y-1">
                  <div className="text-muted font-sans text-[11px]">Total Queries (24h)</div>
                  <div className="text-base font-bold text-white">{telemetry.databaseHealth.totalQueries}</div>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-border space-y-1">
                  <div className="text-muted font-sans text-[11px]">Query Error Rate</div>
                  <div className="text-base font-bold text-success">{telemetry.databaseHealth.errorRatePercent}%</div>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-border space-y-1">
                  <div className="text-muted font-sans text-[11px]">Avg Query Duration</div>
                  <div className="text-base font-bold text-info">{telemetry.databaseHealth.avgDurationMs} ms</div>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-border space-y-1">
                  <div className="text-muted font-sans text-[11px]">Slow Queries (&gt;1000ms)</div>
                  <div className="text-base font-bold text-warning">{telemetry.databaseHealth.slowQueryCount}</div>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-surface-elevated border border-border space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                <div className="p-3 rounded-xl bg-black/40 border border-border space-y-1">
                  <div className="text-muted font-sans text-[11px]">Webhooks Processed</div>
                  <div className="text-base font-bold text-white">{telemetry.webhookHealth.totalReceived}</div>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-border space-y-1">
                  <div className="text-muted font-sans text-[11px]">Success Rate</div>
                  <div className="text-base font-bold text-success">{telemetry.webhookHealth.successRatePercent}%</div>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-border space-y-1">
                  <div className="text-muted font-sans text-[11px]">Failure Rate</div>
                  <div className="text-base font-bold text-muted">{telemetry.webhookHealth.failureRatePercent}%</div>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-border space-y-1">
                  <div className="text-muted font-sans text-[11px]">Avg Processing Time</div>
                  <div className="text-base font-bold text-info">{telemetry.webhookHealth.avgDurationMs} ms</div>
                </div>
              </div>
            </div>
          </div>
